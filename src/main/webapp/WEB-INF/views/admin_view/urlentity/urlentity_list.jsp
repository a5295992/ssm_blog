...
<!--工具栏  -->
		...
		<table id="dg" class="easyui-datagrid" title="路径列表" style="width:100%;height:100%"
			data-options="
				iconCls: 'icon-edit',
				singleSelect: true,
				toolbar: '#tb',
		...
		<thead>
			<tr>
				...
				<th data-options="field:'id'" width="80" sortable="true">ID</th>
				<th data-options="field:'description',editor:'textbox',align:'center'" width="20%" >描述</th>
				<th data-options="field:'url',editor:'textbox',align:'center'" width="15%">请求地址</th>
				<th data-options="field:'methodType',width:60,align:'center',editor:{type:'checkbox',options:{on:'GET',off:'POST'}}">请求方法</th>
				<th data-options="field:'testParams',editor:'textbox',align:'center'" width="15%">测试参数</th>
				<th data-options="field:'_operate',width:80,align:'center',formatter:formatOper">测试</th>  
			</tr>
		</thead>
	</table>
	<div id="tb" style="height:auto">
		...
		<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'fa fa-refresh',plain:true" onclick="flushDb()">同步数据</a>
		<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'fa fa-link',plain:true" onclick="testUrl()">一键测试</a>
	...
	var urlquery =headpath+ "/a/urlentity/query";//查询统一访问路径
	var urladd =headpath+"/a/urlentity/add"; //增删改 统一访问路径
	var entitysName = "urlEntitys";   //实体集合名
	var flushUrl    = headpath+"/a/urlentity/flushDb";
	...
	var jsonData = {};
		...
		function getChanges(rows,delFlag){
			...
			for (var int = 0; int < rows.length; int++) {
				...
				if(delFlag!=null){
					...
					urlEntity['methodType']=dg.methodType;
		...
		function flushDb(){
			$.messager.confirm("提示", "该操作会刷新数据库 确定吗？", function(data){
				if(data){
					var result = submitAjax({},flushUrl,"GET",true);
		...
		function testUrl(index){
		    $('#dg').datagrid('selectRow',index);// 关键在这里  
			    ...
			    if (row){  
					...
					var url=urlEn.url;
					var methodType = urlEn.methodType;
					if(methodType==""){
						methodType="GET";
					}
					var testParams = urlEn.testParams;
					//console.log("url: "+url+"  methodType :  "+methodType+"  testParams: "+testParams);
					$.messager.defaults={ok:"确认",cancel:"取消"}; 
					var result=null;
					$.messager.prompt("参数设置", "默认:"+testParams, function(data){
						if(data){
							result = submitAjax(data,headpath+url,methodType,false);
						}else{
							result = submitAjax(testParams,headpath+url,methodType,false);
						}
					});
					if(result!=null){
						alert("测试完毕");
